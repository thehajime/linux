...
int lkl_load_config_json(struct lkl_config *cfg, char *jstr)
{
	...
	for (pos = 1; pos < jp.toknext; pos++) {
		...
		} else if (jsoneq(jstr, &toks[pos], "mounts") == 0) {
			cfgptr = &cfg->mounts;
		} else {
...
void lkl_config_mount_pre(const char *mounts)
{
	char *saveptr = NULL, *token = NULL;
...
void lkl_config_mount_post(const char *mounts)
{
	char *saveptr = NULL, *token = NULL;
	char *key = NULL, *value = NULL;
	char strings[256];
...
int lkl_load_config_pre(struct lkl_config *cfg)
{
	...
	if (cfg->mounts)
		lkl_config_mount_pre(cfg->mounts);
...
int lkl_load_config_post(struct lkl_config *cfg)
{
	...
	if (cfg->mounts)
		lkl_config_mount_post(cfg->mounts);
